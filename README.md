# Hi, I'm Chris Lam

Product engineer and designer building **y30**, voice AI for older adults and their caregivers. I design it, build it, and ship it, no handoff.

I work at the intersection of product, design, and engineering, and I use AI as a force multiplier to move fast while owning the architecture and product calls.

> 👀 **Open to senior IC roles in voice AI, healthcare, or product engineering.** Full-time or contract. [whoischrislam@gmail.com](mailto:whoischrislam@gmail.com)

## Building now

At **y30**, practical voice AI for older adults and their caregivers:
- Real-time voice pipelines and AI agents.
- Products that create useful caregiver signal, not just transcripts.
- An FSM safety engine with deterministic crisis response, so the model never generates crisis language.
- Built for healthcare reality: messy workflows, safety constraints, and real adoption.

## Background

14 years across product, design, and engineering. Early employee at three IPOs plus an acquisition.

- **Co-founder, PlaySesh.** Started as a social gaming play against loneliness, then pivoted to a real-time collaborative design tool for indie game devs inside Discord. Grew to ~5,000 users, 1,302 MAU, and 50-minute average sessions, fully organic.
- **GoodRx / Iodine.** Sole designer, pre-IPO. Took coupon conversion from 2% to 5%; data viz featured on ABC News and Good Morning America.
- **Clover Health.** 2nd designer, pre-IPO. Rebuilt the nurse home-visit tool for roughly 7–8× revenue impact.
- **doc.ai / Sharecare, Amazon, and TaskRabbit (acquired by IKEA),** plus other 0→1 and growth-stage work.

## Build stack

- Python, TypeScript, React.
- LiveKit, realtime audio, AI agent pipelines.
- Supabase, Cloudflare Workers, Vercel, Docker.
- Figma, systems thinking, and shipping taste.

## Links

- Portfolio: [whoischrislam.github.io](https://whoischrislam.github.io)
- LinkedIn: [linkedin.com/in/whoischrislam](https://linkedin.com/in/whoischrislam)
- Email: [whoischrislam@gmail.com](mailto:whoischrislam@gmail.com)
- Based in Honolulu, with focused build sprints in San Francisco.

Outside work: board games (I'm designing one), 20+ years dancing, and roller skating.
